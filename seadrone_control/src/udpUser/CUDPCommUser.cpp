...
//        NSLog(@"th %d rpm %d",i,sensorData_.m_rpm_[i]);
    ...
    {
      if (robot_data_->batteryVolt_ == 0.0) 
      { //this is our first valid measurement. Set to the instantaneous value.
        robot_data_->batteryVolt_ = sensorData_.battery_volt_;
      } 
      else 
      { //take a running average with past measurements
...
/** load command to the udp data struct */
  void CUDPCommUser::updateUserCommandData() 
  {
    ...
    commandData_.goal_euler_angles_[0] = robot_data_->goal_Eular_[0];
    commandData_.goal_euler_angles_[1] = robot_data_->goal_Eular_[1];
    commandData_.goal_euler_angles_[2] = robot_data_->goal_Eular_[2];

    //commandData_.goal_depth_ = robot_data_-> goal_depth_;
...
/** print out the sensor data */
  void CUDPCommUser::printSensorData(tUdpSensorData *data) 
  {
    ...
    std::cout << "goal depth = " << data->goal_depth_ << std::endl;
    std::cout << "goal_euler = " << data->goal_Eular_[0] << std::endl;
    std::cout << "imu =";
    for (int i = 0; i < 6; i++)
    ...
    std::cout << data->battery_volt_ << std::endl << std::endl;
    std::cout << "-----------------------------" << std::endl;
    std::cout << "Camera = " << robot_data_->gimbal_Pitch_ << std::endl;
    std::cout << "LED[top/bot] = " << "[" << robot_data_->led_brightness_top_ << " / " << robot_data_->led_brightness_bot_ << "]" << std::endl;

  }
} /* namespace orobotix */
//motor of seadrone 
/*                    camera
  data->c_rpm_[2]                     data->c_rpm_[3]
                    data->c_rpm_[4]
  data->c_rpm_[0]                     data->c_rpm_[1]
*/